...
export default function AdminInventoryReservations() {
  ...
  const [reservations, setReservations] = useState<(InventoryReservation & { product?: Product; warehouse?: Warehouse; branch?: Branch })[]>([]);
  ...
  useEffect(() => {
    (async () => {
      ...
      setReservations((res ?? []).map((r) => ({ ...(r as InventoryReservation), product: pMap[(r as InventoryReservation).product_id], warehouse: wMap[(r as InventoryReservation).warehouse_id ?? ''], branch: bMap[(r as InventoryReservation).branch_id ?? ''] })));
      setLoading(false);
    })();
  }, []);
  ...
  const expired = reservations.filter((r) => r.status === 'expired' || (r.status === 'active' && r.expires_at && new Date(r.expires_at) < new Date()));
  ...
  return (
    <div>
      ...
      {loading ? <Skeleton className="h-64" /> : reservations.length === 0 ? (
        <EmptyState icon={<Lock className="w-10 h-10" />} title="No inventory reservations" description="Reservations are created automatically when stock is held for orders." />
      ) : (
        <DataTable<InventoryReservation & { id: string; product?: Product; warehouse?: Warehouse; branch?: Branch }>
          rows={reservations}
          columns={[
            { key: 'reservation_number', label: 'Reservation #', render: (r) => <span className="font-mono text-gold-300">{r.reservation_number}</span> },
            ...
            { key: 'quantity', label: 'Qty', render: (r) => <span className="font-semibold text-ink-100">{r.quantity}</span> },
            { key: 'location', label: 'Location', render: (r) => <span className="text-ink-300 text-sm">{r.warehouse?.name ?? r.branch?.name ?? '—'}</span> },
            { key: 'status', label: 'Status', render: (r) => <Badge color={r.status === 'active' ? 'gold' : r.status === 'released' ? 'neutral' : r.status === 'fulfilled' ? 'accent' : 'warning'}>{r.status}</Badge> },
            { key: 'expires_at', label: 'Expires', render: (r) => <span className="text-ink-400 text-xs">{r.expires_at ? formatDateTime(r.expires_at) : '—'}</span> },
            { key: 'created_at', label: 'Created', render: (r) => <span className="text-ink-400 text-xs">{formatDateTime(r.created_at)}</span> },
            { key: 'actions', label: '', render: (r) => r.status === 'active' ? <Button size="sm" variant="ghost" onClick={() => release(r.id)}><Unlock className="w-4 h-4" /> Release</Button> : null },
          ]}
        />
      )}
    </div>
